...
`packages/backend-adapter-sdk`, `packages/codex-cli-bridge`,
`packages/opencode-bridge`, and `examples/adapters/mock-backend` are original
repository code and currently inherit the repository MPL-2.0 license. The SDK
...
local session state. If a future PR imports code from OpenCode, preserve MIT
