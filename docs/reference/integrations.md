...
| OpenAI Codex CLI | Upstream-informed external CLI/process adapter research | `packages/codex-cli-bridge` records capability metadata, execution-policy mapping, Codex thread/item JSONL normalization, sanitized fixture replay, stdin prompt command boundary, and Tauri native process boundary. Local dogfood covers success/failure JSONL, invalid-model classification, usage, command output, and `file_change`; resume, approvals, and MCP tool fidelity remain open. Raw Codex log/session persistence remains excluded; no vendored source. |
