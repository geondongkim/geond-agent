...
| OpenAI Codex CLI | Upstream-informed external CLI/process adapter research | `packages/codex-cli-bridge` records capability metadata, execution-policy mapping, Codex thread/item JSONL normalization, sanitized fixture replay, stdin prompt command boundary, and Tauri native process boundary. Raw Codex log/session persistence remains excluded; no vendored source. |
